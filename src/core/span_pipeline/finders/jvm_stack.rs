...
pub(crate) struct JvmStackFinder {
    ...
    frame: StyleId,
    file: StyleId,
    unknown_source: StyleId,
    line_number: StyleId,
}

impl JvmStackFinder {
    pub fn new(config: JvmStackTraceConfig, palette: &mut Palette) -> Self {
        ...
        Self {
            ...
            exception: palette.intern(config.exception),
            frame: palette.intern(config.frame),
            file: palette.intern(config.file),
            unknown_source: palette.intern(config.unknown_source),
            line_number: palette.intern(config.line_number),
        }
    }
}
...
mod tests {
    ...
    fn spans(input: &str) -> Vec<(usize, usize, StyleId)> {
    ...
    fn span_text<'a>(input: &'a str, span: &(usize, usize, StyleId)) -> &'a str {
        &input[span.0..span.1]
    }
    ...
    fn header_does_not_match_inside_frame() {
        ...
        let f = make_finder();
        let result = spans(input);
        let header_styles: Vec<&StyleId> = result.iter().map(|s| &s.2).filter(|s| **s == f.exception).collect();
        assert!(
